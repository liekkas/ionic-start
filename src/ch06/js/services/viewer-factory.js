...
(function (app) {
    ...
    app.factory('ViewerFactory', ViewerFactory);


    /* @ngInject */
    function ViewerFactory($timeout,Viewer,EventBus,ThreejsSet) {

        var home,intersected;
        ...
        function init(params) {
            ...
            $timeout(function () {
                EventBus.emitMsg('appReady');
            }, ThreejsSet.LOAD_DELAY);
            animate();
        }
        ...
        function makeSelection(mouse) {
            ...
            if(intersected.length > 0){
                ...
                EventBus.emitMsg('objectSelected', intersected[0])
            } else {
                intersected = null;
        ...
        function loadOBJMTL(info){
            home.wrangler.loadOBJMTL(info.obj, info.mtl, info.name,info.scale,info.positionX,info.positionY,info.positionZ,info.rotateX,info.rotateY,info.rotateZ);
        }
        /**
         * Load OBJ file.
        ...
        function loadPLY(info){
            home.wrangler.loadPLY(info.url,info.name);
        }
